...
# Um cliente para a rede Solana e um cliente para todas as chamadas HTTP (Jupiter, Gecko, etc.)
solana_client = None
http_client = None
...
# ... (O resto das funções de compra e venda permanecem iguais, pois já usam execute_swap)
...
async def fetch_geckoterminal_ohlcv(pair_address, timeframe, limit=60):
    ...
    try:
        res = await http_client.get(url, timeout=10.0) # Usa o cliente global
...
async def fetch_dexscreener_real_time_price(pair_address):
    ...
    try:
        res = await http_client.get(url, timeout=5.0) # Usa o cliente global
        res.raise_for_status()
...
async def discover_and_filter_pairs():
    ...
    for page in range(1, 11):
        ...
        try:
            res = await http_client.get(url, timeout=20.0) # Usa o cliente global
            res.raise_for_status()
...
# ... (O resto do seu código, como analyze_and_score_coin, autonomous_loop, etc., permanece igual)
...
async def run_bot(update, context):
    ...
    # Inicia o cliente HTTP persistente
